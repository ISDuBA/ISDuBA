...
func (fm *ForwardManager) runTargets(ctx context.Context) {
	for _, target := range fm.targets {
		...
		// Avoid starting multiple go routines for the same target
		target.running = true
...
func (fm *ForwardManager) uploadDocuments(ctx context.Context, target *target, documentIDs []int64) {
	for _, documentID := range documentIDs {
		...
		fm.uploadDocument(ctx, documentString, documentID, target)
	}
}

func (fm *ForwardManager) uploadDocument(ctx context.Context, doc string, documentID int64, target *target) {
	req, err := fm.buildRequest(doc, "test", target)
	if err != nil {
		slog.Error("building forward request failes", "err", err)
		return
	}
...
func (fm *ForwardManager) fetchNewDocuments(ctx context.Context, url string, publisher *string) ([]int64, error) {
	...
	if publisher != nil {
		wherePublisher = "WHERE publisher = $2 "
	}
	...
	if err := fm.db.Run(
		...
		func(rctx context.Context, conn *pgxpool.Conn) error {
			fetchSQL := `SELECT documents_id FROM events_log ` +
				...
				`(SELECT id FROM documents ` + wherePublisher +
				`AND not in (SELECT documents_id FROM forwarded_documents WHERE url = $2) ORDER BY time DESC`
			rows, _ := conn.Query(rctx, fetchSQL, url, publisher)
			var err error
...
func (fm *ForwardManager) ForwardDocument(target *config.ForwardTarget, documentID int64) error {
