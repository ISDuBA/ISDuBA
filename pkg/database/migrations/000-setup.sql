...
-- This is a table with extracted nums of the product names extracted from the product tree.
...
-- This is a table with extracted nums of the product ids extracted from the product tree.
...
CREATE TABLE stored_queries (
    ...
    default_query bool                NOT NULL DEFAULT FALSE,
...
CREATE TABLE sources (
    ...
    checksum               bytea,
    checksum_ack           timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP - '1 second'::interval,
    checksum_updated       timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CHECK(name <> ''),
    CHECK(url <> ''),
...
CREATE TABLE forwarders_queue (
    upload_order  int           NOT NULL GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE aggregators (
    ...
    checksum_ack      timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP - '1 second'::interval,
    checksum_updated  timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CHECK(url LIKE '%/aggregator.json')
);
...
CREATE TRIGGER update_products_id_and_names_texts
    AFTER INSERT
    ON documents
    EXECUTE FUNCTION update_products_id_and_names_texts();
...
--
INSERT INTO sources (id, name, url, checksum_ack, checksum_updated)
    VALUES (0, 'manual imports', 'manual.invalid', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
