...
func (c *Controller) viewForwardTargets(ctx *gin.Context) {
	...
	for id, target := range c.cfg.Forwarder.Targets {
		targets = append(targets, forwardTarget{ID: id, URL: target.URL})
	}
...
func (c *Controller) forwardDocument(ctx *gin.Context) {
	...
	targetID, ok := parse(ctx, toInt64, ctx.Query("target"))
	if !ok {
		targetID = 0
	}
	...
	if len(c.cfg.Forwarder.Targets) <= int(targetID) {
		ctx.JSON(http.StatusNotFound, gin.H{"error": "could not find target with specified id"})
		return
	}
	if err := c.fm.ForwardDocument(&c.cfg.Forwarder.Targets[int(targetID)], documentID); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": err})
		return
	}
