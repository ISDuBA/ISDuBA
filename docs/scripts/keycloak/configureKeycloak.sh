...
live_import() {
  ...
  if sudo /opt/keycloak/bin/kcadm.sh get 'http://localhost:8080/admin/realms' | grep -F -q '"realm" : "isduba",'; then 
    echo "Realm isduba already exists, skipping import."
  else
    sudo /opt/keycloak/bin/kcadm.sh create realms -f "$realm_file"
  fi
}

# TODO: what if keycloak is running, but does not have an admin user yet?
if $keycloak_running; then
  echo "keycloak is assumed to be running; importing isduba realm via the admin API."
  live_import
else
  if curl --silent --head -fsS "http://localhost:$live/health/ready"; then
    ...
    echo "Importing isduba realm before starting keycloak..."
    sudo /opt/keycloak/bin/kc.sh import --file "$realm_file" --override false
