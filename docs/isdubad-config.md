...
- `default_aggregator`: The aggregator that is displayed, if no aggregator is configured. Defaults to `"https://wid.cert-bund.de/.well-known/csaf-aggregator/aggregator.json"`.
...
| `ISDUBA_AGGREGATORS_DEFAULT_AGGREGATOR` | `aggregators default_aggregator`     |
